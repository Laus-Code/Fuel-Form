...
const SuccessBar = (p: IBarProps): JSX.Element => {
    return (<MessageBar messageBarType={MessageBarType.success} onDismiss={p.onDismiss}>
        {p.message}
    </MessageBar>);
};

const ErrorBar = (p: IBarProps): JSX.Element => {
    return (<MessageBar messageBarType={MessageBarType.error} onDismiss={p.onDismiss}>
        {p.message}
    </MessageBar>);
};
...
export default class FuelForm extends React.Component<IFuelFormProps, IFuelFormState> {
    ...
    public render(): React.ReactElement<IFuelFormProps> {
        const {context, companyNames, suppliers} = this.props;
        ...
        if (distance < 10 && distance !== 0.1) {
            distanceErrorMessage = "Dystans musi się zawierać w wartościach pomiędzy 10 a 9999";
        }
        ...
        if (company && registrationNumber && supplier && cardNumber && distance >= 10 && route && startDate && endDate && justification && supervisor) {
            ...
            <Stack tokens={defaultStackToken} styles={stackStyle}>
                ...
                <Stack horizontal tokens={defaultStackToken} verticalAlign="center">
                    ...
                    {!formForUser && !personOnList ? (<div>
                        <Stack horizontal>
                            <TextField
                                ...
                                errorMessage={!name && showErrorMessages ? defaultErrorMessage : ""}
                            />
                            <TextField
                                label="Nazwisko"
                                placeholder="Wprowadź Nazwisko"
                                onChange={this.onChangeSurname}
                                errorMessage={!surname && showErrorMessages ? defaultErrorMessage : ""}
                            />
                        </Stack>
                        <TextField
                            label="adres email"
                    ...
                    {!formForUser && personOnList ? (<PeoplePicker
                        ...
                        errorMessage={!driver && showErrorMessages ? defaultErrorMessage : ""}
                    />) : null}
                </Stack>
                ...
                <Stack horizontal tokens={defaultStackToken}>
                    ...
                    <Dropdown
                        ...
                        errorMessage={!company && showErrorMessages ? defaultErrorMessage : ""}
                    />
                </Stack>
                ...
                <Stack horizontal tokens={defaultStackToken}>
                    <Dropdown
                        ...
                        errorMessage={!supplier && showErrorMessages ? defaultErrorMessage : ""}
                    />
                    {mask || mask === "" ? (<MaskedTextField
                        label="Numer karty"
                        mask={mask}
                        onChange={this.onChangeCardNumber}
                        placeholder="Wpisz numer karty"
                        errorMessage={!cardNumber && showErrorMessages ? defaultErrorMessage : ""}
                    />) : (<TooltipHost
                        content={mask === undefined ? "Wybierz dostawcę przed wpisaniem numeru karty" : ""}
                    >
                        <TextField
                            label="Numer karty"
                            onChange={this.onChangeCardNumber}
                ...
                <Stack horizontal tokens={defaultStackToken}>
                    <MaskedTextField
                        ...
                        maskChar=""
                        errorMessage={distance === 0.1 && showErrorMessages ? defaultErrorMessage : distanceErrorMessage}
                    ...
                    <MaskedTextField
                        ...
                        errorMessage={!registrationNumber && showErrorMessages ? defaultErrorMessage : ""}
                    />
                </Stack>
                <TextField
                    ...
                    errorMessage={!route && showErrorMessages ? defaultErrorMessage : ""}
                />
                <Stack horizontal tokens={defaultStackToken}>
                    <DatePicker
                ...
                <TextField
                    ...
                    errorMessage={!justification && showErrorMessages ? defaultErrorMessage : ""}
                />
                {formSent && success ? (<SuccessBar
                    onDismiss={() => {
                        this.setState({formSent: false});
                    }}
                    message={"Wniosek złożony poprawnie. Dziękujemy!"}
                />) : null}
                {formSent && !success ? (<ErrorBar
                    onDismiss={() => {
                        this.setState({formSent: false});
                    }}
                    message={"Wykryto błąd przy składaniu wniosku! Spróbuj jeszcze raz. W przypadku dalszych problemów skontaktuj się z administracją."}
                />) : null}
                {showErrorBar ? (<ErrorBar
                    onDismiss={() => {
                        this.setState({showErrorBar: false});
                    }}
                    message={!startDate || !endDate ? "Wykryto błąd przy składaniu wniosku! Wybierz datę rozpoczęcia i zakończenia i spróbuj ponownie." : "Wykryto błąd przy składaniu wniosku! Upewnij się, że wszystkie pola są dobrze wypełnione."}
                />) : null}
            </Stack>
            <br/>
            <DefaultButton
    ...
    private onChangePersonNotOnList = (ev: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked: boolean): void => {
        this.setState({
            personOnList: !isChecked,
        });
    };

    private onChangePersonChoice = (ev: React.FormEvent<HTMLElement | HTMLInputElement>, option: IChoiceGroupOption): void => {
        this.setState({
    ...
    private onChangeName = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, name?: string): void => {
        this.setState({
            name: name,
        });
    };

    private onChangeSurname = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, surname?: string): void => {
        this.setState({
            surname: surname,
        });
    };

    private onChangeEmail = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, email?: string): void => {
        this.setState({
            email: email,
        });
    };
    ...
    private onChangeCompany = (ev: React.FormEvent<HTMLDivElement>, option: IDropdownOption): void => {
        this.setState({
            company: option.text,
        });
    };

    private onChangeRegistrationNumber = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, number?: string): void => {
        this.setState({
            registrationNumber: number,
        });
    };

    private onChangeSupplier = (ev: React.FormEvent<HTMLDivElement>, option: IDropdownOption): void => {
        this.setState({
            supplier: option.key as number,
        });
    };

    private onChangeCardNumber = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, cardNumber?: string): void => {
        this.setState({
            cardNumber: cardNumber,
        });
    };

    private onChangeDistance = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, distance?: string): void => {
        this.setState({
    ...
    private onChangeRoute = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, route?: string): void => {
        this.setState({
            route: route,
        });
    };
    ...
    private onChangeJustifaction = (ev: React.FormEvent<HTMLElement | HTMLTextAreaElement>, justification?: string): void => {
        this.setState({
            justification: justification,
        });
    };

    private onClickSubmit(state: IFuelFormState, context: WebPartContext, formCorect: boolean): void {
        this.setState({
            showErrorBar: true, showErrorMessages: true,
        });
        ...
        this.sp.web.lists
            ...
            .catch((error) => {
                ...
                this.sp.web.lists
                    ...
                    .items.add({
                    Title: context.pageContext.user.email, errorMessage: errorMessage,
                })
                    .catch((error) => alert(error));
                sentCorrectly = false;
            });
        this.setState({
            formSent: true, success: sentCorrectly, showErrorBar: false,
        });
    }
}
